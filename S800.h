...
class S800  {
 ...
 public:
  S800(TRandom* ran, histo_sort * Histo0);
  S800(TRandom* ran, histo_read * Histo1);
  S800(TRandom* ran, histo_sort * Histo0, histo_read * Histo1, readBeam *Beams0, runOptions *opt0);
  void Reset();
  ~S800();
...
int fail;
  ...
  float Chargeslope[2][NPads];
  float Chargeintercept[2][NPads];
  float ObjCorr[2]; //1 - obj/mrad  2 - obj/mm
  int gravity_width;

  vector<int> CRDCCalRunsX[2];
  vector<float> CRDCCalSlopeX[2];
  vector<float> CRDCCalOffsetX[2];

  vector<int> CRDCCalRunsY[2];
  vector<float> CRDCCalSlopeY[2];
  vector<float> CRDCCalOffsetY[2];
